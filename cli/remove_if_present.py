...
if len(sys.argv) < 6:
	print >> sys.stderr, "Usage", sys.argv[0], "<filter key file, multicolumn AND> <key col> <num records> <file to filter> <filter columns>"
	sys.exit(1)
...
filter_keys = [set() for k in xrange(len(filter_columns))]
...
with open(filter_file) as f:
	for line in f:
		...
		match = True
		for k in xrange(len(filter_columns)):
